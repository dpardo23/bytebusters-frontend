...
// Asegúrate de que UserData en ProfileHeader tenga un campo 'id'
import type { UserData } from "./ProfileHeader" 
import { ExperienceForm } from "./ExperienceForm"
import { EducationForm } from "./EducationForm"

interface ProfileEditFormProps {
  initialUser: UserData & { id?: string | number }; // Heredamos el ID
}
...
export function ProfileEditForm({ initialUser }: ProfileEditFormProps) {
  ...
  // Si el usuario no tiene datos previos, inician en blanco listos para editarse
  ...
  const handleSave = async () => {
    ...
    try {
      const userId = initialUser.id || 1; // Usamos el ID heredado (fallback a 1 para pruebas)

      // 1. Guardar Información Básica (El backend debe crear el Profile si no existe)
      ...
      const responseHero = await fetch(`/api/profile/hero-section`, { 
        method: 'PATCH', 
        body: formDataToSend 
      });
      ...
      // 2. Guardar Estado de Disponibilidad
      const backendStatus = formData.status === 'active' ? 'A' : formData.status === 'busy' ? 'B' : 'N';
      const statusParams = new URLSearchParams({ 
        status: backendStatus, 
        message: "", // Puedes agregar un input para esto si lo deseas
        incognito: String(formData.status === 'incognito') 
      });
      await fetch(`/api/profile/status?${statusParams.toString()}`, { method: 'PUT' });
      ...
      // Redirección directa al Home tras guardar exitosamente
      alert("¡Perfil creado/guardado con éxito!");
      navigate('/');
      
    } catch (error) {
      console.error("Error al guardar:", error);
      alert("Hubo un problema al intentar guardar tu perfil. Inténtalo de nuevo.");
    } finally {
      setIsLoading(false);
    }
  }
