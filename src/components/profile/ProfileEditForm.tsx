...
export function ProfileEditForm({ initialUser }: ProfileEditFormProps) {
  ...
  const [originalData, setOriginalData] = useState<ExtendedUserData>(initialUser) // Guarda los datos originales para poder "Cancelar"
  ...
  // ----------------------------------------------------------------------
  // TAREA: OBTENER DATOS DE LA BASE DE DATOS AL CARGAR LA PÁGINA
  // ----------------------------------------------------------------------
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        const [statusRes, bioRes, heroRes] = await Promise.all([
          ...
          fetch(`/api/profile/hero-section/${currentUserId}`) // <-- NUEVO
        ]);
        ...
        if (heroRes.ok) {
          ...
          if (heroData.photoBase64) {
            setPhotoPreview(heroData.photoBase64); // Pintamos la foto de la BD
          }
        }
  ...
  // ----------------------------------------------------------------------
  // AUTO-GUARDADO DE BIOGRAFÍA (Solo funciona si estamos en modo edición)
  // ----------------------------------------------------------------------
  useEffect(() => {
  ...
  const handleCancelEdit = () => {
    // Restaurar los datos originales y salir del modo edición
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formDataToSend.append('data', new Blob([JSON.stringify({ name: formData.name, professionalTitle: formData.headline })], { type: "application/json" }));
      ...
      setOriginalData(formData); // Actualizamos el respaldo con los nuevos datos
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 mt-8 mb-20">
      ...
      {/* SECCIÓN 5: EDUCACIÓN */}
      {/* Pasamos el estado isEditing para que EducationForm sepa si debe mostrar los botones de borrar/editar o solo la lista */}
